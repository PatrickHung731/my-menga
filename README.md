...
```
 ...
 [t] 調對白/旁白字級大小       [p] 發布網站 / 推上 GitHub
```

**字太小 / 氣泡擋到角色？** 按 `[t]` 選一話 → 選字級 `1 小 / 2 中 / 3 大 / 4 特大`。
只重拼頁（幾秒，不重畫圖）。字變大氣泡也變大，若擋到角色就用 `[4]` 把那句的 `pos` 搬到別的角落。

**做完不喜歡畫風/角色怎麼辦？**
- 按 `[s]` **改整部畫風**：選新畫風（七龍珠/海賊王/火影…）→ 故事對白全保留，只把畫全部重畫成新風格（連角色臉都用同一顆種子重繪，不會變成別人）。很花時間但一鍵搞定。
- 按 `[c]` **改某個角色**：選角色 → 直接 Enter＝只換一張臉重抽；或輸入新的英文外觀（例：`orange fur, green eyes, torn ear`）＝重新設計。系統只會重畫「有他出場」的格，其他格不動。
- 兩個做完都會問要不要順便發布上線。
